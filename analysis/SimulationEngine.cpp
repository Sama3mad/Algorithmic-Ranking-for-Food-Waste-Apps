...
void SimulationEngine::run_day_simulation(int num_customers) {
...
void SimulationEngine::run_multi_day_simulation(int num_days, int num_customers_per_day) {
    ...
    for (int day = 1; day <= num_days; day++) {
        ...
        // Note: We keep customers map to preserve history across days
        ...
        // Run day simulation
        // Note: Customer IDs will be 0 to (num_customers_per_day-1) each day
        // This is fine because we're aggregating metrics, not tracking individual customers across days
        run_day_simulation(num_customers_per_day);
        
        // Clear customers map after each day to avoid ID conflicts
        // (Customer history is already recorded in metrics)
