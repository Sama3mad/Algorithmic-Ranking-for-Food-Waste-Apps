...
int CustomerDecisionSystem::process_customer_arrival(Customer& customer,
    ...
    // Update impression counts for all displayed stores (for ANDREW algorithm)
...
int CustomerDecisionSystem::select_store(const Customer& customer,
    ...
    for (size_t i = 0; i < adjusted_scores.size(); i++) {
        // Filter out stores that are too far (score < -50 indicates distance threshold violation)
        // and stores below the customer's threshold
